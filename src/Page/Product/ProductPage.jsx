...
import { no_prod_img } from "../../Assets/index";
...
const ProductPage = () => {
  ...
  const [filterItemTexts, setFilterItemTexts] = React.useState([]);
  ...
  React.useEffect(() => {
    const getAllSearchedResults = async () => {
      try {
        if (
          ...
          const searchResponse = await searchService.searchProducts(
            searchParams.get("search")
          );
          ...
          if (
            ...
            throw new Error("No Search params found in search.");
          }
        }
      } catch (error) {
        setSearchStatus(false);
    ...
    getAllSearchedResults();
  }, [searchParams]);

  const addItemsToFilter = (event, data, key) => {
    const ele = event.target.checked;
    const filterText = [...filterItemTexts];
    ...
    if (ele) {
      if (findIdl === undefined) {
        idlFrs.push(data);
        fltrItem[key] = idlFrs;
        filterText.push(data);
      }
    } else {
      if (findIdl !== undefined) {
        filterText.splice(filterText.indexOf(data), 1);
        idlFrs.splice(idlFrs.indexOf(data), 1);
        fltrItem[key] = idlFrs;
      }
    }
    setFilterItem(fltrItem);
    setFilterItemTexts(filterText);
  };
  ...
  //     setFilterItemTexts((filterItemTexts) => filterItemTexts.filter((chip) => chip !== chipToDelete));
  // };
          ...
          <Box className="sidebar">
            <Box className="filter">
              <Box sx={{ m: 2 }}>
                ...
                <Paper
                  sx={{
                    display: filterItemTexts.length !== 0 ? "flex" : "none",
                          ...
                          <FormControlLabel
                            ...
                            control={
                              <Checkbox
                                sx={{ "& .MuiSvgIcon-root": { fontSize: 20 } }}
                                onChange={(e) =>
                                  addItemsToFilter(e, brnd.name, "brand")
                                }
                              />
                            }
                            label={
                          ...
                          <FormControlLabel
                            ...
                            control={
                              <Checkbox
                                id={x}
                                sx={{ "& .MuiSvgIcon-root": { fontSize: 20 } }}
                                onChange={(e) => addItemsToFilter(e, x, "type")}
                              />
                            }
                            label={
                              <Typography
                ...
                <Grid container spacing={1}>
                  {products.length > 0 ? (
                    products
                      .filter((item) => {
                        if (
                          (filterItem.idealFor.length !== 0 &&
                            filterItem.idealFor.includes(
                              item.productDetails.idealFor
                            )) ||
                          (filterItem.brand.length !== 0 &&
                            filterItem.brand.includes(item.brand.name)) ||
                          (filterItem.occasion.length !== 0 &&
                            filterItem.occasion.includes(
                              item.productDetails.occasion
                            )) ||
                          (filterItem.type.length !== 0 &&
                            filterItem.type.includes(item.productDetails.type))
                        ) {
                          return true;
                        }
                        if (
                      ...
                      .map((shoes) => (
                        <Grid key={shoes.title} item xs={3}>
                          <Card
                            sx={{ cursor: "pointer" }}
                            onClick={() => {
                              window.open(`/product/details/${shoes.id}`);
                            }}
                          >
                            <CardHeader
