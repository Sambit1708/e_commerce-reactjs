const Products = {
    
    trendings : [
        {
            name: 'Nike Air Max Dn',
            imgPath: require('../../Assets/images/trending/air-max-dn-shoes.png')
        },
        {
            name: "MAPF1 A3rokart Mid Men's Motorsport Sneakers",
            imgPath: require('../../Assets/images/trending/motorsport-sneakers.png')
        },
        {
            name: 'Nike Air Max 270',
            imgPath: require('../../Assets/images/trending/air-max-270-shoes.png')
        },
        {
            name: "MAPF1 A3rokart Mid Men's Motorsport Sneakers",
            imgPath: require('../../Assets/images/trending/Scuderia-Ferrari-Carbon-Cat.png')
        }
    ],

    productDetailsHeader: ['Color', 'Type', 'Occasion', 'Ideal For', 'Model Name'],

    productDetails: {
        "color": "PUMA Black-Sheen Green",
        "Outer Material": "Synthetic upper",
        "Model Name": "Modern Trendy Sooes",
        "Ideal for": "Men",
        "Occasion": "Casual",
        "Types For Casual": "Sneakers",
        "Net Quantity": 1
    },
    ratingAndReviews: [
        {
            rating: 4,
            review: "It is a nice product",
            user: "Rajib Takhur"
        },
        {
            rating: 5,
            review: "Very nice product",
            user: "Sambit Mahanty"
        },
        {
            rating: 3,
            review: "It is kind of average boot",
            user: "Kamal Hasan"
        },
        {
            rating: 4,
            review: "It was overall ok product",
            user: "Rolex Sir"
        }
    ],
    brands: ["Nike", "Puma", "Adidas", "Campus"],
    brandHeads: [
        {
            id: 'brand',
            label: 'Brand Name',
        },
        {
            id: 'pLink',
            label: 'Product Link',
        },
        {
            id: 'desc',
            label: 'Description',
        }
    ],
    occasion: ["Casual", "Formal", "Party", "Sports"],

    shoesProducts: [
        {
            name: "RYSTA PRO Running Shoes For Men",
            brand: "CAMPUS",
            price: 545,
            imgPath: require("../../Assets/images/Products/crysta_pro_running.jpg")
        },
        {
            name: "MIKE (N) Running Shoes For Men (Grey)",
            brand: "CAMPUS",
            price: 849,
            imgPath: require('../../Assets/images/Products/mike_running_shooes.jpg')
        },
        {
            name: "HURRICANE Running Shoes For Men",
            brand: "CAMPUS",
            price: 699,
            imgPath: require("../../Assets/images/Products/hurricane_running_shooes.jpg")
        },
        {
            name: "FIRST Running Shoes For Men (White)",
            brand: "CAPMUS",
            price: 1375,
            imgPath: require("../../Assets/images/Products/first_running_shoes.jpg")
        },
        {
            name: "Dash-Sprint Running Shoes For Men (Off White)",
            brand: "ADIDAS",
            price: 2352,
            imgPath: require("../../Assets/images/Products/dash-sprint-running-shoes.jpg"),
            occasion: "Sports"
        },
        {
            name: "Grand Court 2.0 Sneakers For Men  (White)",
            brand: "ADIDAS",
            price: 3760,
            imgPath: require("../../Assets/images/Products/grand_court2.jpg"),
            occasion: "Casual"
        },
        {
            name: "ELEMENT 1.0 M Casuals For Men  (Black)",
            brand: "ADIDAS",
            price: 1899,
            imgPath: require("../../Assets/images/Products/element1m.jpg"),
            occasion: "Formal"
        },
        {
            name: "Nike Air Max Dn",
            brand: "NIKE",
            price: 13995,
            imgPath: require("../../Assets/images/Products/air-max-dn-shoes.png"),
            occasion: "Sports"
        },
        {
            name: "Ebernon Low Premium Running Shoes For Men (White)",
            brand: "NIKE",
            price: 7095,
            imgPath: require("../../Assets/images/Products/air-max-dn-shoes.png"),
            occasion: "Casual"
        },
        {
            name: "Firefly Sneakers For Men (Blue)",
            brand: "PUMA",
            price: 1329,
            imgPath: require("../../Assets/images/Products/firefly_sneakers.jpg"),
            occasion: "Casual"
        },
        {
            name: "ST Runner v3 Mesh Running Shoes For Men (Black)",
            brand: "PUMA",
            price: 1935,
            imgPath: require("../../Assets/images/Products/st_runner_v3 .jpg"),
            occasion: "Sports"
        },
        {
            name: "Technner Womens Sneakers For Women (Pink)",
            brand: "PUMA",
            price: 2499,
            imgPath: require("../../Assets/images/Products/technner_womens_sneakers.jpg"),
            occasion: "Casual"
        },
        {
            name: "Hybrid Fuego Wns Walking Shoes For Women (Black)",
            brand: "PUMA",
            price: 4199,
            imgPath: require("../../Assets/images/Products/hybrid_fuego_wns.jpg"),
            occasion: "Sports"
        },
    ],

    brandDetails: [
        {
            id: 1,
            title: "Adidas",
            description: `The company was started by Adolf Dassler in his mother's house. He was joined by his elder brother Rudolf in 1924 under the name Gebrüder Dassler Schuhfabrik ("Dassler Brothers Shoe Factory"). Dassler assisted in the development of spiked running shoes (spikes) for multiple athletic events`,
            quantity: 5 
        },
        {
            id: 2,
            title: "Puma",
            description: `Puma SE is a German multinational corporation that designs and manufactures athletic and casual footwear, apparel, and accessories, headquartered in Herzogenaurach, Bavaria, Germany. Puma is the third largest sportswear manufacturer in the world. The company was founded in 1948 by Rudolf Dassler`,
            quantity: 6 
        },
        {
            id: 3,
            title: "Nike",
            description: `Nike, Inc. is an American athletic footwear and apparel corporation headquartered near Beaverton, Oregon, United States. It is the world's largest supplier of athletic shoes and apparel and a major manufacturer of sports equipment, with revenue in excess of US$46 billion in its fiscal year 2022`,
            quantity: 7 
        },
        {
            id: 4,
            title: "Campus",
            description: `Campus is India’s largest sports and athleisure footwear brand. Incorporated in 2006, Campus Activewear is one of the leading players in organized sports & casual footwear sector in India. Since 2016, the flagship brand "Campus", has been the largest sports and athleisure footwear brand in India, in both volume and value terms`,
            quantity: 8 
        },
        {
            id: 5,
            title: "Board",
            description: `The company was started by Adolf Dassler in his mother's house. He was joined by his elder brother Rudolf in 1924 under the name Gebrüder Dassler Schuhfabrik ("Dassler Brothers Shoe Factory"). Dassler assisted in the development of spiked running shoes (spikes) for multiple athletic events`,
            quantity: 5 
        },
        {
            id: 6,
            title: "Sams",
            description: `The company was started by Adolf Dassler in his mother's house. He was joined by his elder brother Rudolf in 1924 under the name Gebrüder Dassler Schuhfabrik ("Dassler Brothers Shoe Factory"). Dassler assisted in the development of spiked running shoes (spikes) for multiple athletic events`,
            quantity: 5 
        }
    ],

    ProductHeaders: [
        {
            id: 'avatar',
            label: 'Image',
        },
        {
          id: 'title',
          label: 'Title',
        },
        {
          id: 'price',
          label: 'Price',
        },
        {
          id: 'brand',
          label: 'Brand',
        },
        {
          id: 'rating',
          label: 'Ratings',
        },
        {
          id: 'description',
          label: 'Description',
        },
    ],
    productDetails2: [
        {
            id: "56025448-613b-460d-8ddc-1729c7814244",
            title: "Barbie Crush",
            price: 1235.0,
            details: "size:6#quantity1:10#hello:Hello World I am Sambit Khandai",
            description: `The company was started by Adolf Dassler in his mother's house. He was joined by his elder brother Rudolf in 1924 under the name Gebrüder Dassler Schuhfabrik ("Dassler Brothers Shoe Factory"). Dassler assisted in the development of spiked running shoes (spikes) for multiple athletic events`,
            rating: 4.3,
            imagePath: null,
            brand: {
                id: "6009215f-dc9d-48bd-9000-7f5040be606a",
                name: "PUMA",
                description: "Hello"
            },
            productSizes: [
                {
                    id: "ae67c90b-ea2d-4982-863f-2c4b1202fb46",
                    size: 6,
                    quantity: 10
                },
                {
                    id: "c2f6be75-c123-4aaf-840b-6a31b27cb5f1",
                    size: 7,
                    quantity: 10
                }
            ],
            productDetails: {
                color: "White, Black",
                modelName: "Casual Sneaker Shoes for Men | Advanced Rounded Front, Elevated Softness",
                idealFor: "Men",
                occasion: "Casual",
                type: "Sneakers",
            }
        },
        {
            id: "56025448-613b-460d-8ddc-1729c7814245",
            title: "Barbie Crush",
            price: 1235.0,
            details: "size:6 quantity1:10 ",
            description: "Ok",
            rating: 0.0,
            imagePath: null,
            brand: {
                id: "6009215f-dc9d-48bd-9000-7f5040be606a",
                name: "PUMA",
                description: "Hello"
            },
            productSizes: [
                {
                    id: "ae67c90b-ea2d-4982-863f-2c4b1202fb46",
                    size: 6,
                    quantity: 10
                },
                {
                    id: "c2f6be75-c123-4aaf-840b-6a31b27cb5f1",
                    size: 7,
                    quantity: 10
                }
            ],
            productDetails: {
                color: "White, Black",
                modelName: "Casual Sneaker Shoes for Men | Advanced Rounded Front, Elevated Softness",
                idealFor: "Men",
                occasion: "Casual",
                type: "Sneakers",
            }
        },
        {
            id: "56025448-613b-460d-8ddc-1729c7814246",
            title: "Barbie Crush",
            price: 1235.0,
            details: "size:6 quantity1:10 ",
            description: "Ok",
            rating: 0.0,
            imagePath: null,
            brand: {
                id: "6009215f-dc9d-48bd-9000-7f5040be606a",
                name: "PUMA",
                description: "Hello"
            },
            productSizes: [
                {
                    id: "ae67c90b-ea2d-4982-863f-2c4b1202fb46",
                    size: 6,
                    quantity: 10
                },
                {
                    id: "c2f6be75-c123-4aaf-840b-6a31b27cb5f1",
                    size: 7,
                    quantity: 10
                }
            ],
            productDetails: {
                color: "White, Black",
                modelName: "Casual Sneaker Shoes for Men | Advanced Rounded Front, Elevated Softness",
                idealFor: "Men",
                occasion: "Casual",
                type: "Sneakers",
            }
        },
        {
            id: "56025448-613b-460d-8ddc-1729c7814247",
            title: "Barbie Crush",
            price: 1235.0,
            details: "size:6 quantity1:10 ",
            description: "Ok",
            rating: 0.0,
            imagePath: null,
            brand: {
                id: "6009215f-dc9d-48bd-9000-7f5040be606a",
                name: "PUMA",
                description: "Hello"
            },
            productSizes: [
                {
                    id: "ae67c90b-ea2d-4982-863f-2c4b1202fb46",
                    size: 6,
                    quantity: 10
                },
                {
                    id: "c2f6be75-c123-4aaf-840b-6a31b27cb5f1",
                    size: 7,
                    quantity: 10
                }
            ],
            productDetails: {
                color: "White, Black",
                modelName: "Casual Sneaker Shoes for Men | Advanced Rounded Front, Elevated Softness",
                idealFor: "Men",
                occasion: "Casual",
                type: "Sneakers",
            }
        },  
        {
            id: "56025448-613b-460d-8ddc-1729c7814248",
            title: "Barbie Crush",
            price: 1235.0,
            details: "size:6 quantity1:10 ",
            description: "Ok",
            rating: 0.0,
            imagePath: null,
            brand: {
                id: "6009215f-dc9d-48bd-9000-7f5040be606a",
                name: "PUMA",
                description: "Hello"
            },
            productSizes: [
                {
                    id: "ae67c90b-ea2d-4982-863f-2c4b1202fb46",
                    size: 6,
                    quantity: 10
                },
                {
                    id: "c2f6be75-c123-4aaf-840b-6a31b27cb5f1",
                    size: 7,
                    quantity: 10
                }
            ],
            productDetails: {
                color: "White, Black",
                modelName: "Casual Sneaker Shoes for Men | Advanced Rounded Front, Elevated Softness",
                idealFor: "Men",
                occasion: "Casual",
                type: "Sneakers",
            }
        },     
        {
            id: "56025448-613b-460d-8ddc-1729c7814249",
            title: "Barbie Crush",
            price: 1235.0,
            details: "size:6 quantity1:10 ",
            description: "Ok",
            rating: 0.0,
            imagePath: null,
            brand: {
                id: "6009215f-dc9d-48bd-9000-7f5040be606a",
                name: "PUMA",
                description: "Hello"
            },
            productSizes: [
                {
                    id: "ae67c90b-ea2d-4982-863f-2c4b1202fb46",
                    size: 6,
                    quantity: 10
                },
                {
                    id: "c2f6be75-c123-4aaf-840b-6a31b27cb5f1",
                    size: 7,
                    quantity: 10
                }
            ],
            productDetails: {
                color: "White, Black",
                modelName: "Casual Sneaker Shoes for Men | Advanced Rounded Front, Elevated Softness",
                idealFor: "Men",
                occasion: "Casual",
                type: "Sneakers",
            }
        }
    ],

    orderDetails: [
        {
            id: "56025448-613b-460d-8ddc-1729c7814245",
            totalAmount: 1235.0,
            orderStatus: "CONFIRMED",
            createdDate: "25-06-2023",
            updateDate: "25-06-2023",
            user: "56025448-613b-460d-8ddc-1729c7814241"
        },
        {
            id: "56025448-613b-460d-8ddc-1729c7814244",
            totalAmount: 1235.0,
            orderStatus: "PENDING",
            createdDate: "25-06-2023",
            updateDate: "25-06-2023",
            user: "56025448-613b-460d-8ddc-1729c7814242"
        },
        {
            id: "56025448-613b-460d-8ddc-1729c7814243",
            totalAmount: 1235.0,
            orderStatus: "ON_HOLD",
            createdDate: "25-06-2023",
            updateDate: "25-06-2023",
            user: "56025448-613b-460d-8ddc-1729c7814243"
        },
        {
            id: "56025448-613b-460d-8ddc-1729c7814242",
            totalAmount: 1235.0,
            orderStatus: "SHIPPED",
            createdDate: "25-06-2023",
            updateDate: "25-06-2023",
            user: "56025448-613b-460d-8ddc-1729c7814244"
        },
        {
            id: "56025448-613b-460d-8ddc-1729c7814246",
            totalAmount: 1235.0,
            orderStatus: "DELIVERED",
            createdDate: "25-06-2023",
            updateDate: "25-06-2023",
            user: "56025448-613b-460d-8ddc-1729c7814241"
        },
        {
            id: "56025448-613b-460d-8ddc-1729c7814247",
            totalAmount: 1235.0,
            orderStatus: "CANCELLED",
            createdDate: "25-06-2023",
            updateDate: "25-06-2023",
            user: "56025448-613b-460d-8ddc-1729c7814241"
        },
    ],
    userImg: {
        data: "data:image/png;base64,"
    },

    cartItem: [
        {
            id: "demo",
            quantity: "demo",
            createDate: "demo",
            updateDate: "demo",
            product: {
                id: "demo",
                title: "demo",
                price: "demo",
                description: "demo",
                rating: "demo",
                imageName: "demo",
                imagePath: "demo",
                createDate: "demo",
                updateDate: "demo",
                brand: {
                    id: "demo",
                    name: "demo",
                    description: "demo",
                    createDate: "demo",
                    updateDate: "demo",
                },
                productSizes: [{
                    id: "demo",
                    size: 0,
                    quantity: 0,
                    createDate: "demo",
                    updateDate: "demo"
                }],
                productDetails: {
                    id: "demo",
                    color: "demo",
                    modelName: "demo",
                    idealFor: "demo",
                    occasion: "demo",
                    type: "demo",
                    createDate: "demo",
                    updateDate: "demo",
                }
            },
            productSize: {
                id: "demo",
                size: 0,
                quantity: 0,
                createDate: "demo",
                updateDate: "demo"
            }
        }
    ],

    addressItem: [
        {
            id: 1,
            address: "27th Main Rd, near IQRA School, Old Madiwala, Jay Bheema Nagar, 1st Stage, BTM 1st Stage",
            pin: "560068",
            state: "Karnataka",
            city: "Bengaluru",
            houseNumber: "",
            roadName: "27th Main Road",
            landMark: "near IQRA School",
            type: "0",
            currentAddress: true,
            deliverTo: "Sambit Khandai",
            phoneNumber: "9861114128",
            createDate: "2024-05-31 06:15:09.097000",
            updateDate: "2024-05-31 06:15:09.097000",
        },
        {
            id: 2,
            address: "27th Main Rd, near IQRA School, Old Madiwala, Jay Bheema Nagar, 1st Stage, BTM 1st Stage",
            pin: "560068",
            state: "Karnataka",
            city: "Bengaluru",
            houseNumber: "",
            roadName: "27th Main Road",
            landMark: "near IQRA School",
            type: "0",
            currentAddress: false,
            deliverTo: "Sambit Khandai",
            phoneNumber: "9861114128",
            createDate: "2024-05-31 06:15:09.097000",
            updateDate: "2024-05-31 06:15:09.097000",
        },
        {
            id: 3,
            address: "27th Main Rd, near IQRA School, Old Madiwala, Jay Bheema Nagar, 1st Stage, BTM 1st Stage",
            pin: "560068",
            state: "Karnataka",
            city: "Bengaluru",
            houseNumber: "",
            roadName: "27th Main Road",
            landMark: "near IQRA School",
            type: "0",
            currentAddress: false,
            deliverTo: "Sambit Khandai",
            phoneNumber: "9861114128",
            createDate: "2024-05-31 06:15:09.097000",
            updateDate: "2024-05-31 06:15:09.097000",
        },
    ]
}

export default Products;