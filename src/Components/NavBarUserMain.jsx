...
const NavBarUserMain = () => {
  ...
  const searchProduct = (event) => {
    ...
    const searchValue = searchRef.current.search.value.trim();

    if (searchValue === "") {
      return;
    }
    navigate(`/product?search=${searchValue}`, { replace: true });
  };
    ...
    >
      <Toolbar sx={{ display: "flex", justifyContent: "space-between" }}>
        ...
        >
          ...
          <Box component="form" ref={searchRef} onSubmit={searchProduct}>
            <Paper
              sx={{
                p: "2px 4px",
                display: "flex",
